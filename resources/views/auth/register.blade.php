...
@section('content')
    ...
    <div class="bg-white flex flex-col space-y-10 justify-center items-center" style="height: 700px">
        <div class="bg-white w-96 shadow-xl rounded p-4">
            ...
            <form action="{{ route("register_process") }}" class="space-y-5 mt-4" method="POST">
                @csrf
                <input name="name" type="text" class="w-full h-12 border border-gray-800 rounded px-3 @error('name') border-red-500 @enderror" placeholder="Имя пользователя" />
